...
public class AccountService {
	...
	public double getBalanceCheck(int user_id, String authorization) {
		Double balance;
		balance = restTemplate.exchange(BASE_URL + "account/" + user_id + "/balanceCheck",
	    		HttpMethod.GET, makeAccountEntity(authorization), Double.class).getBody();
		return balance;
	}
	...
	public void sendBucks(int fromUserId, int toUserId, double amount, String authorization) {
		 restTemplate.exchange(BASE_URL + "account/transferwithparam?toUserId="+toUserId+"&fromUserId="+fromUserId+"&amount="+amount,
		    		HttpMethod.POST, makeAccountEntity(authorization), Transfer[].class).getBody();
		}
