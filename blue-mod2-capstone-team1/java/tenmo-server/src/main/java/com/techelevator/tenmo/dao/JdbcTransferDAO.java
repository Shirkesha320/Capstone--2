...
public class JdbcTransferDAO implements TransferDAO {
	...
	private static final int TRANSFER_TYPE_ID = 2;
	private static final int TRANSFER_STATUS_ID = 2;
	...
	public void sendBucks(int userId, int toUserId, double amount) {
		...
		jdbcTemplate.update(sqlTo, amount, toUserId);
		
		String sqlTransfer = "INSERT INTO transfers (transfer_type_id, transfer_status_id, "
				+ "account_from, account_to, amount) " +
				"VALUES (?, ?, (SELECT account_id FROM accounts WHERE user_id = ?), "
				+ "(SELECT account_id FROM accounts WHERE user_id = ?), ?)";
		
		jdbcTemplate.update(sqlTransfer, TRANSFER_TYPE_ID, TRANSFER_STATUS_ID, userId, toUserId, amount );
	}
	...
	public List<Transfer> viewTransfers(int userId) {
		...
		// adding transfers where user RECEIVED money
				sql = "SELECT transfer_id, amount, transfer_type_desc, username AS To, " +
					"(SELECT username FROM users WHERE user_id = " +
					"(SELECT user_id FROM accounts WHERE account_id = t.account_from)) AS From FROM  users " +
					"JOIN accounts a ON a.user_id = users.user_id " +
					"JOIN transfers t ON t.account_to = a.account_id " +
					"JOIN transfer_types ty ON ty.transfer_type_id = t.transfer_type_id " +
					"WHERE users.user_id= ?";	
			
				SqlRowSet moreResult = jdbcTemplate.queryForRowSet(sql,userId);
				
				while(moreResult.next()) {
					Transfer transfer = mapRowToTransfer(moreResult);
					transfers.add(transfer);
				}
		return transfers;
	}
